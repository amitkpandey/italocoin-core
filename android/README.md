...
# Base environnement
        ...
        cd italocoin/utils/build_scripts
        docker build -f android32.Dockerfile -t italocoin-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t italocoin-gui-android .
        docker create -it --name italocoin-gui-android italocoin-gui-android bash

# Get the apk

        docker cp italocoin-gui-android:/opt/android/italocoin-core/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/italocoin-core/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
