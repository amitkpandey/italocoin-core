; Italocoin Lithium Luna GUI Wallet Installer for Windows
; Copyright (c) 2014-2018, The Italocoin Project
; See LICENSE

[Setup]
AppName=Italocoin GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.12.0.0
DefaultDirName={pf}\Italocoin GUI Wallet
DefaultGroupName=Italocoin GUI Wallet
UninstallDisplayIcon={app}\italocoin-wallet-gui.exe
...
; As of version 0.12.0.0, the Italocoin .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Italocoin.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Italocoin GUI wallet
Source: "bin\italocoin-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Italocoin GUI wallet log file
...
Source: "italocoin-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Italocoin CLI wallet
Source: "bin\italocoin-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Italocoin wallet RPC interface implementation
Source: "bin\italocoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Italocoin daemon
Source: "bin\italocoind.exe"; DestDir: "{app}"; Flags: ignoreversion

; Italocoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "italocoin-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Italocoin blockchain utilities
Source: "bin\italocoin-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\italocoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0 and after
; Source: "bin\italocoin-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\italocoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bititalocoin');
  s := 'The default folder to store the Italocoin blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\bititalocoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "italocoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Italocoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\italocoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\italocoin-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Italocoin GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\italocoin-wallet-gui.exe"
...
Name: "{group}\Utilities\Italocoin Daemon"; Filename: "{app}\italocoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\italocoin-wallet-cli.exe"; WorkingDir: "{userdocs}\Italocoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Italocoin\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\italocoin-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\italocoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM italocoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\italocoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\italocoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\italocoin-project\italocoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\italocoin-project\italocoin-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
