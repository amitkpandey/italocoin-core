...
ITALOCOIN_DIR=italocoin
ITALOCOIND_EXEC=italocoind
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/italocoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    ITALOCOIND_EXEC=italocoind.exe
fi
...
get_tag
popd
echo "var GUI_ITALOCOIN_VERSION = \"$TAGNAME\"" >> version.js

cd build
qmake ../italocoin-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy italocoind to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$ITALOCOIN_DIR/bin/$ITALOCOIND_EXEC $BIN_PATH
fi
